---
title: "Analyzing RNA-seq data with DES2ACT"
...
abstract: >
  ...
  variability. The package DES2ACT provides methods to test for
  ...
  FASTQ files.
  DES2ACT package version: `r packageVersion("DES2ACT")`
output:
...
vignette: >
  %\VignetteIndexEntry{Analyzing RNA-seq data with DES2ACT}
...
**Note:** if you use DES2ACT in published research, please cite:

> Love, M.I., Huber, W., Anders, S. (2014)
> Moderated estimation of fold change and dispersion for RNA-seq data with DES2ACT.
...
analysis. There are a variety of steps upstream of DES2ACT that result
...
## How to get help for DES2ACT

Any and all DES2ACT questions should be posted to the 
**Bioconductor support site**, which serves as a searchable knowledge
base of questions and answers:

<https://support.bioconductor.org>

Posting a question and tagging with "DES2ACT" will automatically send
...
computational performance of *DES2ACT* and building an interface to his
*glmGamPoi* package.

We have benefited in the development of *DES2ACT* from the help and
...
DES2ACT and its developers have been partially supported by funding from
...
As input, the DES2ACT package expects count data as obtained, e.g.,
...
is important to provide count matrices as input for DES2ACT's
statistical model [@Love2014] to hold, as only the count values allow
assessing the measurement precision correctly. The DES2ACT model
...
The object class used by the DES2ACT package to store the read counts 
...
on what pipeline was used upstream of DES2ACT to generated counts or
estimated counts:
...
Our recommended pipeline for *DES2ACT* is to use fast transcript 
abundance quantifiers upstream of DES2ACT, and then to create
gene-level count matrices for use with DES2ACT 
...
Note that the tximport-to-DES2ACT approach uses *estimated* gene
...
We import the necessary quantification data for DES2ACT using the
...
*SummarizedExperiment* that can be loaded easily into *DES2ACT* using
...
order.  DES2ACT will not make guesses as to which column of the count
matrix belongs to which row of the column data, these must be provided
to DES2ACT already in consistent order.
...
can be easily input into DES2ACT as follows. First we load the package
...
the DES2ACT functions, there are two reasons which make pre-filtering
...
count modeling within DES2ACT. It can also
...
alphabetical order. Then, if you never tell the DES2ACT functions which
...
DES2ACT provides a function *collapseReplicates* which can
...
`?DESeq` and in the Methods section of the DES2ACT publication [@Love2014]. 
...
glmGamPoi's interface in DES2ACT requires use of `test="LRT"` and
...
adjustment of DES2ACT results.  For more details, please see the
...
For advanced users, note that all the values calculated by the DES2ACT 
...
In DES2ACT, the function *plotMA* shows the log2
...
shrinkage for certain datasets. In DES2ACT version 1.18, we include two
...
* `ashr` is the adaptive shrinkage estimator from the
  ...
  Here DES2ACT uses the ashr option to fit a mixture of Normal distributions to
  form the prior, with `method="shrinkage"`.
* `normal` is the the original DES2ACT shrinkage estimator, an adaptive
...
accommodated in DES2ACT by including in the design any known batch
...
package. The *DES2ACTReport* function should be run on a 
*DESeqDataSet* that has been processed by the *DESeq* function.
For more details see the manual page for *DES2ACTReport* 
and an example vignette in
the [regionReport](http://bioconductor.org/packages/regionReport) 
package. 

**Glimma** Interactive visualization of DES2ACT output, 
...
**pcaExplorer** Interactive visualization of DES2ACT output,
...
In fact, DES2ACT can analyze any possible experimental design that can
...
be easily included in the DES2ACT design to control for technical
...
for examples of using RUV or SVA in combination with DES2ACT. 
...
also archived
[here](https://htmlpreview.github.io/?https://github.com/frederikziebell/science_tweetorials/blob/master/DES2ACT_many_samples.html)
by Frederik Ziebell.
...
time. See the DES2ACT paper for more discussion on the differences
[@Love2014].
...
terms. A 
[tutorial](https://github.com/tavareshugo/tutorial_DES2ACT_contrasts) 
describing the use of numeric contrasts for DES2ACT explains a general
...
DES2ACT offers two kinds of hypothesis tests: the Wald test, where
...
DES2ACT are good for ranking genes by "effect size", that is the log2
...
[previous section](#altshrink), the original DES2ACT shrinkage
...
largest observed LFCs. As you can read in the DES2ACT paper, under the
...
LFC is also why DES2ACT's shrinkage estimator is not recommended for
...
DES2ACT shrinkage method is not really compatible with computing s-values.
...
and their standard errors from upstream methods (here, DES2ACT's MLE).
...
The DES2ACT developers and collaborating groups have published
recommendations for the best use of DES2ACT for single-cell datasets,
which have been described first in @Berge2018. Default values for
DES2ACT were designed for bulk data and will not be appropriate for
...
* One important concern for single-cell data analysis is the size of the datasets and
  associated processing time. To address the speed concerns, *DES2ACT* provides an 
...
account of these in the DES2ACT model. This allows for the DES2ACT
...
combining *zinbwave* and *DES2ACT* package functions in the
*zinbwave* vignette. We also have an example of ZINB-WaVE + DES2ACT
...
outliers are set aside by DES2ACT, or if there are sufficient samples,
...
The *results* function of the DES2ACT package performs independent
...
The version of DES2ACT which was used to construct the
...
vignettes how to create matrices which can be used by DES2ACT.
...
This design can be analyzed by DES2ACT but requires a bit of
...
# Theory behind DES2ACT

## The DES2ACT model 

The DES2ACT model and all the steps taken in the software
are described in detail in our publication [@Love2014],
and we include the formula and descriptions in this section as well.
The differential expression analysis in DES2ACT uses a generalized
...
An option in DES2ACT is to provide maximum *a posteriori*
...
The main changes in the package *DES2ACT*, compared to the (older)
...
## Methods changes since the 2014 DES2ACT paper
...
* In version 1.16 (November 2016), the log2 fold change 
  ...
  shrinkage did not affect statistical testing, DES2ACT has become used
...
* For the calculation of the beta prior variance, instead of
  ...
  distribution, DES2ACT now uses the weighted quantile function
...
DES2ACT relies on the negative binomial distribution to make
...
to a Normal distribution as are the Wald statistics for the DES2ACT
package.
...
using `contrast`, DES2ACT uses *expanded model matrices* to produce
shrunken log2 fold change estimates where the shrinkage is independent
of the choice of reference level. In all other cases, DES2ACT uses
...
described in the DES2ACT paper. Using type `normal` with `coef` uses
...
## How can I get support for DES2ACT?
...
## How can I get unfiltered DES2ACT results?
...
The transformations implemented in *DES2ACT*, `vst` and `rlog`, compute
...
(nor does *DES2ACT* have a way to specify which covariates are nuisance
...
## Can I use DES2ACT to analyze paired samples?
...
[above](#theory) and the DES2ACT paper for full details.
...
## Can I run DES2ACT to contrast the levels of many groups?

DES2ACT will work with any kind of design specified using the R
...
DES2ACT. Users might consider first removing genes with very few
reads, as this will speed up the fitting procedure.

## Can I use DES2ACT to analyze a dataset without replicates?

No. This analysis is not possible in *DES2ACT*.
...
[DES2ACT model](#theory) in this document.

## Is there an official Galaxy tool for DES2ACT?

Yes. The repository for the DES2ACT tool is
...
## I want to benchmark DES2ACT comparing to other DE tools.

One aspect which can cause problems for comparison is that, by default,
DES2ACT outputs `NA` values for adjusted *p* values based on 
independent filtering of genes which have low counts.
This is a way for the DES2ACT to give extra
...
## I have trouble installing DES2ACT on Ubuntu/Linux...

"*I try to install DES2ACT, but I get an error trying to
