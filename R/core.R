############################################################
#
# DES2ACT organization of R files
#
...
# AllGenerics .... the generics defined in DES2ACT
...
#' DES2ACT package for differential analysis of count data
#' 
#' The DES2ACT package is designed for normalization,
...
#' \code{vignette("DES2ACT")}, or the workflow linked to on the first page
#' of the vignette.
#' 
...
#' for RNA-seq data with DES2ACT. Genome Biology, 15:550.
#' \url{https://doi.org/10.1186/s13059-014-0550-8}
#'
#' @author Michael Love, Wolfgang Huber, Simon Anders
#' 
#' @docType package
#' @name DES2ACT-package
#' @aliases DES2ACT-package
#' @keywords package
NULL
...
#' low counts before running DES2ACT in parellel: this improves efficiency
...
#' @references
#'
#' Love, M.I., Huber, W., Anders, S. (2014) Moderated estimation of fold change and dispersion for RNA-seq data with DES2ACT. Genome Biology, 15:550. \url{https://doi.org/10.1186/s13059-014-0550-8}
#'
...
DESeq <- function(object, test=c("Wald","LRT"),
  ...
  metadata(object)[["version"]] <- packageVersion("DES2ACT")
...
#' @param type can either be "DES2ACT" or "glmGamPoi". Specifies if the glmGamPoi
...
estimateDispersionsGeneEst <- function(object, minDisp=1e-8, kappa_0=1,
                                       ...
                                       alphaInit=NULL,
                                       type = c("DES2ACT", "glmGamPoi")) {
  
  type <- match.arg(type, c("DES2ACT", "glmGamPoi"))
...
#' prior variance and the Methods section of the DES2ACT manuscript for more detail.
...
#' (function has been copied into DES2ACT to avoid extra dependencies).
...
nbinomWaldTest <- function(object,
  ...
  if (useT) {
    ...
    # the default DES2ACT p-value: use the standard Normal
...
#' matrix X while fitting the GLM
#' @param minmu lower bound on the estimated count while fitting the GLM
#' @param type either "DES2ACT" or "glmGamPoi". If \code{type = "DES2ACT"} a
...
getAndCheckWeights <- function(object, modelMatrix, weightThreshold=1e-2) {
  if ("weights" %in% assayNames(object)) {
    ...
    if (is.null(attr(object, "weightsOK"))) {
      ...
      # most designs are full rank with current version of DES2ACT
      if (full.rank) {
...
# to avoid extra package dependencies in DES2ACT (same license as Hmisc),
