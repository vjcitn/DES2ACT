...
#' (Note \code{filtered_p} R code is now copied into DES2ACT
#' package to avoid gfortran requirements.)
#' In DES2ACT version >= 1.10, the threshold that is chosen is
...
of length 3 to 'contrast' instead of using 'name'")
  ...
  deseq2.version <- packageVersion("DES2ACT")
  if (!attr(object,"betaPrior")) {
    priorInfo <- list(type="none", package="DES2ACT", version=deseq2.version)
  } else {
    betaPriorVar <- attr(object, "betaPriorVar")
    priorInfo <- list(type="normal", package="DES2ACT", version=deseq2.version,
